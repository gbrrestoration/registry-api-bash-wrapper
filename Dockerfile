...
ADD registry-api.sh /usr/bin/registry-api.sh
ADD _registry-api.sh /usr/local/share/zsh/site-functions/_registry-api.sh
ADD registry-api.sh.bash-completion /etc/bash-completion.d/registry-api.sh
RUN chmod 755 /usr/bin/registry-api.sh
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/registry-api.sh\n\
' >> ~/.bashrc
...
\n\
$(tput setaf 3)REGISTRY_API_HOST$(tput sgr0) - server URL, e.g. https://example.com:8080\n\
\n\
